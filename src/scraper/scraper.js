...
// Scrape all departments on the SSC and store the object in courseData.json
async function scrapeDepts() {
    ...
    await Promise.all(promises).then( values => {
        ...
        fs.writeFile(path.join(__dirname, '../data', 'courseData.json'), JSON.stringify(globalData), err => {
            if (err) throw err;
        });
...
function scrapeBuildings() {
    fs.readFile(path.join(__dirname, '../data', 'courseData.json'), 'utf8', async (err, data) => {
        ...
        await Promise.all(promises).then( values => {
            ...
            fs.writeFile(path.join(__dirname, '../data', 'buildings.json'), JSON.stringify(buildingData), err => {
                if (err) throw err;
            });
